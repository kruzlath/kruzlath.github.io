...
function write_old_desc(horny_factor){
    let scp_num=64902
    let SCP="SCP-"+scp_num;
    let desc="";
    if (randint(1,3)>1){
...
function write_new_desc(horny_factor){
    let desc="";
    let which_desc=weightedChoice([0,1],[]);
    if (which_desc==0){
        let chosen_phrase=weightedChoice("Let me out! ","Get out of my head! ","Get out of your head! ",[])
        for (let i=0;i<100;i++){
            desc+=chosen_phrase;
            if (randint(1,10)==1){
                desc+="\n";
  ...
  function myPeriodicTask() {
    ...
    let horny=1.1-Player.ArousalSettings["Progress"]/100; //As horniness increases, this will decrease, can be used to make previously large text appear small
